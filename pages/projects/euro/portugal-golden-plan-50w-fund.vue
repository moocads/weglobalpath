<template>
  <div class="hot-projects-detail-page">
    <header
      :style="{
        background: `linear-gradient(
    89.93deg,
    rgba(34, 52, 92, 0.7) 7.82%,
    rgba(34, 52, 92, 0) 102.35%
  ),url(${data.thumbnail.url})`,
      }"
    >
      <div class="wrapper">
        <h1>{{ data.project_name }}</h1>
      </div>
    </header>
    <Breadcrumb project-url="euro" :project-data="data" />
    <div class="nav-wrap">
      <div class="wrapper">
        <ul>
          <a href="#edu-sec-intro">
            <li :class="{ tabActive: activeTab === 1 }" @click="activeTab = 1">
              项目介绍
            </li>
          </a>
          <a href="#edu-sec-advantages">
            <li :class="{ tabActive: activeTab === 2 }" @click="activeTab = 2">
              项目优势
            </li>
          </a>
          <a href="#edu-sec-requirements">
            <li :class="{ tabActive: activeTab === 3 }" @click="activeTab = 3">
              申请条件
            </li>
          </a>
          <a href="#edu-sec-process">
            <li :class="{ tabActive: activeTab === 4 }" @click="activeTab = 4">
              申请流程
            </li>
          </a>
          <a href="#edu-sec-fee">
            <li :class="{ tabActive: activeTab === 5 }" @click="activeTab = 5">
              项目费用
            </li>
          </a>
        </ul>
      </div>
    </div>
    <section class="sec-main">
      <div class="wrapper">
        <div class="image-16-9">
          <img :src="data.thumbnail.url" alt="" />
        </div>
        <div class="main-info">
          <div v-if="data.project_header_info !== null">
            <h4>所属省份</h4>
            <p>
              {{ data.project_header_info.province }}
            </p>
          </div>
          <div v-if="data.project_header_info !== null">
            <h4>移民类型</h4>
            <p>
              {{ data.project_header_info.type }}
            </p>
          </div>
          <div v-if="data.project_header_info !== null">
            <h4>居住要求</h4>
            <p>
              {{ data.project_header_info.requirement }}
            </p>
          </div>
          <div v-if="data.project_header_info !== null">
            <h4>办理周期</h4>
            <p>
              {{ data.project_header_info.duration }}
            </p>
          </div>
          <div v-if="data.project_header_info !== null">
            <h4>身份卡类别</h4>
            <p>
              {{ data.project_header_info.id_type }}
            </p>
          </div>
          <div v-if="data.project_header_info !== null">
            <h4>办理费用</h4>
            <p>
              {{ data.project_header_info.fee }}
            </p>
          </div>
        </div>
      </div>
    </section>
    <section
      id="edu-sec-intro"
      class="sec-intro sec-general sec-bg-gray anchor"
    >
      <div class="wrapper">
        <h2>项目介绍</h2>
        <vue-markdown class="intro-markdown">{{
          data.introduction_richtext
        }}</vue-markdown>
      </div>
    </section>
    <section id="edu-sec-advantages" class="sec-advantages sec-general anchor">
      <div class="wrapper">
        <h2>项目优势</h2>
        <ul>
          <li v-for="(highlight, index) in data.hightlight_points" :key="index">
            <img
              src="/img/Projects/list-square.png"
              alt="projects list square icon"
            />
            {{ highlight.point }}
          </li>
        </ul>
      </div>
    </section>
    <section
      id="edu-sec-requirements"
      class="sec-requirements sec-general sec-bg-gray anchor"
    >
      <div class="wrapper">
        <h2>申请条件</h2>
        <ul>
          <div v-for="(items, index) in requirementsData" :key="index">
            <li v-for="(i, index) in items.requirement_points" :key="index">
              <img
                src="/img/Projects/checkmark.png"
                alt="projects list checkmark icon"
              />
              {{ i.point }}
            </li>
          </div>
        </ul>
      </div>
    </section>
    <section id="edu-sec-process" class="sec-process sec-general anchor">
      <div class="wrapper">
        <h2>申请流程</h2>
        <div class="process-wrap">
          <a-timeline>
            <a-timeline-item v-for="(item, index) in data.process" :key="index">
              <div slot="dot" class="step-num">
                <span>{{ index + 1 }}</span>
              </div>
              {{ item.step }}
            </a-timeline-item>
          </a-timeline>
        </div>
      </div>
    </section>
    <section id="edu-sec-fee" class="sec-fee sec-general sec-bg-gray anchor">
      <div class="wrapper">
        <h2>项目费用</h2>
        <p class="mobile-only">* 左右滑动来查看表格</p>
        <div class="scroll-wrapper">
          <table class="fee-table">
            <thead>
              <th>费用项目</th>
              <th>金额及说明</th>
              <th>收取单位</th>
              <th>何时交纳</th>
            </thead>
            <tbody v-for="(value, key, index) in tableContents" :key="index">
              <tr>
                <th class="table-section-title" colspan="4">{{ key }}</th>
              </tr>
              <tr v-for="(d, i) in value" :key="i">
                <th v-if="isRowSpan(key, i)" rowspan="2" v-html="d[0]"></th>
                <th v-else-if="!isHeaderHidden(key, i)" v-html="d[0]"></th>
                <td class="wrap" v-html="d[1]"></td>
                <td v-html="d[2]"></td>
                <td class="wrap" v-html="d[3]"></td>
              </tr>
            </tbody>
          </table>
          <p>
            **以上是客户所需花费是根据 2020年最新税法调整后的价格
            ，如官方修改收费标准或增加相关收费项目，具体金额以实际发生为准。
          </p>
        </div>
      </div>
    </section>
    <OtherProjectsSection bgWhite="true" />
  </div>
</template>

<script>
import VueMarkdown from "vue-markdown";

export default {
  head() {
    return {
      title: `加拿大移民 | ${this.data.project_name} | 加彼岸出国咨询`,
      meta: [
        {
          hid: "description",
          name: "description",
          content: `加拿大移民 | ${this.data.project_name} | 加彼岸出国咨询`,
        },
      ],
    };
  },
  data() {
    return {
      activeTab: 1,
      tableContents: {
        商签费用: [
          [
            "签证费",
            "约 705.5元人民币／人<br/>价格有波动 ，以使馆出具的收据为准",
            "葡萄牙签证中心",
            "递交时支付",
          ],
          [
            "保险费",
            "80岁以下 1-7天：150元/人；<br/>8-10天： 150元/人；<br/>11-15天： 180元/人；<br/>16-20天： 210元/人",
            "保险公司",
            "购买时支付",
          ],
          ["公证费", "按中国当地公证处的收费标准", "公证处", "做公证时支付"],
          [
            "翻译费",
            "按中国当地翻译公司的收费标准",
            "翻译公司",
            "做翻译时支付",
          ],
          [
            "认证费",
            "参考价格 （ 我方代办） ：单认证约为 220 元／本 ，双认证约为 370 元／本。<br/>实际花费以中国各地认证代办机构出具的发票为准，多退少补",
            "认证代办机构",
            "北京使馆管辖范围的我方代为交纳<br/>上海和广州领馆管辖范围的交给代办机构",
          ],
        ],
        购房费用: [
          [
            "葡萄牙房产",
            "50万欧元以上 ",
            "房产商",
            "购房后支付到葡萄牙个人账户",
          ],
          [
            "定金",
            "2 万欧元或10万人民币",
            "房产商",
            "现金＋信用卡<br/>（ Visa、Mastercard)",
          ],
          [
            "购房律师费",
            "房价的1%.多购买一套房，额外加700欧元.（ 具体以律师收取为准）其中1625欧元部分需要加收23%的VAT，例如50万欧元 的房产，需要缴纳的费用为：5000+1625 * 23％= 5373.75欧元",
            "律师楼",
            "购房后在律师楼支付现金或刷卡",
          ],
          [
            "行政费",
            "1500欧元<br/>（仅限在<span class='detail-highlight'>里斯本</span>开档的客户，包含过户地契，申请及更新税务文件和土地注册证明，申请身份5年内3次免费陪同去里斯本开档）<br/><span class='detail-highlight'>如后期选择在里斯本之外开档，需要缴纳480欧元/天律师陪同费（不含车费）</span>",
            "律师楼",
            "购房后在律师楼支付现金或刷卡",
          ],
          [
            "税务代表费",
            "主申请人及其配偶：600欧元/人/年，夫妻二人：990欧元/年<br/>主申请人及配偶父母：600欧元/人/年，夫妻二人：990欧元/年<br/>18岁以上子女：600欧元/人/年；多于1人，400欧元/人/年<br/>18岁以下子女：300欧元/人/年<br/>（ 具体以税务代表实际收费为准 ：注 ：18岁以上的所有申请人均需办理税号，18岁以下申请可以不用办理。）",
            "税务代表",
            "购房后在律师楼支付现金或刷卡",
          ],
          [
            "房产交易税IMT",
            "居住型房最高约 6% ，商业型约 6. 5% （ 取决于房产类型、房价等 ）",
            "葡萄牙税务局",
            "购房回国后转账给律师代交",
          ],
          ["印花税", "房价的 0.8%", "葡萄牙税务局", "购房回国后转账给律师代交"],
          [
            "附加房产税",
            "针对单套或累计的房产税务价格超过60万欧元的房产，除了正常支付房产税之外，对超出60万欧元的部分加征0.7%的附加 税。即房产计税价值减去600000欧后乘以0.7% ",
            "葡萄牙税务局",
            "通常每年的4月份交纳",
          ],
          [
            "房产市政税IMI",
            "根据房子的位置 、价格等而不同 ，一般城市为房子价值的 0.3%-0.5% ，郊区为 0.8% ，具体由税务代表计算",
            "葡萄牙市政局",
            "通常每年 12 月 31 日之前交纳",
          ],
        ],
        银行相关费用: [
          [
            "账户管理费",
            "Novo Banco Golden Key账户类型，账户管理费约15.6欧元/月",
            "葡萄牙银行",
            "每年据具体情况交纳",
          ],
          ["银行声明的费用", "17.5欧元+VAT", "葡萄牙银行", "银行出具声明时"],
        ],
        房屋保险费用: [
          ["房屋保险费","根据房产类型 、房产价格、保险险种等不同而不同 ，费用约为100欧-300欧元不等/房产/年","保险公司","过户地契后支付，非强制购买，但建议购买"]
        ],
        黄金居留许可申请费用: [
          [
            "黄金居留许可<br/>存档费",
            "约 533欧元 ／人",
            "移民局",
            "新政变更为线上注册文件后在5个工作日内支付该费用。",
          ],
          [
            "黄金居留许可<br/>制卡费",
            "约 5325 欧元／人",
            "移民局",
            "获批后且录完生物信息后才能交制卡费，需在获批后30天内支付，否则需要重新申请。",
          ],
          ["取卡费", "约25欧元 ／人", "移民局", "取卡时"],
          [
            "一年期的医疗保险<br/>（葡萄牙境内有效）",
            "1、在中国购买保险 ： 中国 PICC 保险，每人约人民币 2600 元/人/年<br/> 2、葡萄牙当地保险：在葡萄牙当地保险公司或银行购买，根据年龄不同费用也不同，费用大概100-200欧/人/年",
            "保险公司/银行",
            "国外保险在当地相关机构购买<br/>中国保险我方可代为购买",
          ],
          [
            "生活保证金",
            "主申请人需提供其在葡萄牙当地银行账户的银行对账单。（建议为每位申请人存入至少6000欧元）",
            "葡萄牙当地银行",
            "客户开好账户后汇款",
          ],
          ["公证费", "按中国当地公证处的收费标准", "公证处", "做公证时支付"],
          [
            "翻译费",
            "按中国当地翻译公司的收费标准",
            "翻译公司",
            "做翻译时支付",
          ],
          [
            "双认证费",
            "参考价格 （我方代办）： 民事文件约为 370 元／本、商业文件约为 510 元／ 本<br/>（具体以中国各地认证代办机构出具的发票为准 ，多退少补）",
            "认证代办机构",
            "北京使馆管辖范围的我方代为交纳<br/>上海和广州领馆管辖范围的交给代办机构",
          ],
        ],
        葡萄牙临时居住许可续签的费用和葡萄牙永居申请的费用: [
          [
            "黄金居留许可<br/>续签费用",
            "存档费：约 533欧元 ／人",
            "移民局",
            "递交续签申请时",
          ],
          ["", "制卡费：约 2663 欧元 ／人", "移民局", "续签申请获批后"],
          [
            "永久居住许可<br/>申请费用",
            "存档费：约 533欧元 ／人",
            "移民局",
            "递交申请时",
          ],
          ["", "制卡费：约 7455欧元／人", "移民局", "申请获批后"],
          [
            "永久居住许可<br/>续签费用",
            "存档费：约 533欧元 ／人",
            "移民局",
            "递交续签申请时",
          ],
          ["", "制卡费：约 3728欧元／人", "移民局", "续签申请获批后"],
          ["取卡费", "约25欧元 ／人", "移民局", "取卡时"],
          [
            "税务代表费",
            "主申请人及其配偶：600欧元/人/年，夫妻二人：990欧元/年<br/>主申请人及配偶父母：600欧元/人/年，夫妻二人：990欧元/年<br/>18岁以上子女：600欧元/人/年，多于1人400欧元/人<br/>18岁以下子女：200欧元/人/年<br/>（ 具体以税务代表实际收费为准 ：注 ：18岁以上的所有申请人均需办理税号，18岁以下申请可以不用办理。）",
            "税务代表",
            "等待税务代表通知",
          ],
        ],
      },
    };
  },
  async asyncData({ $axios, route }) {
    const projectData = await $axios.$get(
      `/projects?slug=portugal-golden-plan-50w-fund`
    );
    const data = projectData[0];
    const requirementsData = data.requirements;

    return {
      data,
      requirementsData,
    };
  },
  components: {
    VueMarkdown,
  },
  methods: {
    isRowSpan(key, index) {
      return (
        key === "葡萄牙临时居住许可续签的费用和葡萄牙永居申请的费用" &&
        (index === 0 || index === 2 || index === 4)
      );
    },
    isHeaderHidden(key, index) {
      return (
        key === "葡萄牙临时居住许可续签的费用和葡萄牙永居申请的费用" &&
        (index === 1 || index === 3 || index === 5)
      );
    },
  },
};
</script>
<style lang="scss">
@import "~/assets/scss/hot-projects/general.scss";
</style>
